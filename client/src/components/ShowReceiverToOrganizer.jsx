...
const ShowReceiverToOrganizer = (props) => {
    ...
    const deleteReceiver = async () => {
        try {
            const res = await axios.delete(`http://localhost:3005/api/receiver/deleteReceiver/${props.receiver._id}`, {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const res = await axios.put(`http://localhost:3005/api/receiver/updateReceiver`, newReceiver, {
                headers: { Authorization: `Bearer ${token}` }
            })
            if (res.status === 200) {
                props.setReceiversData(res.data);
                        ...
                        <Dialog
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="p-fluid">
                                <div className="field">
                                    ...
                                    <InputText id="name" defaultValue={receiver.name}  {...register('name', { required: 'שדה חובה' })} className={errors.name ? 'p-invalid' : ''} />
